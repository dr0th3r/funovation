	...
	try {
		...
		if (includeIngredients.length > 0) {
			const ingredientAnyMatch = includeIngredients.map(
				(ingredient) =>
					sql`${recipe.simplifiedIngredients} @> ${JSON.stringify([ingredient])}::jsonb`
			);
